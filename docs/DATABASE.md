...
- **project_files** — metadata only (`file_name`, `file_type`, `file_size`, `storage_key`, `url`, `resource_type`, `format`). Bytes live in Cloudinary; `storage_key` holds the Cloudinary `public_id`.
- **project_images** (added in `db/migrations/001_project_images.sql`, `resource_type`/`format` added in `004_cloudinary_columns.sql`) — cover + gallery images per project: `project_id` (`ON DELETE CASCADE`), `url`, `storage_key` (the Cloudinary `public_id`, needed to delete the underlying asset), `is_cover boolean`, `file_name`, `file_size`, `mime_type`, `resource_type`, `format`, `created_at`. Metadata only, same discipline as `project_files` — no binary/base64 in the row. Bytes live in Cloudinary (`server/src/lib/storage/cloudinary.js`). "At most one cover image per project" is enforced in application logic (`server/src/routes/projects.js`, inside a transaction that unsets the old cover before setting the new one), not a DB constraint — following this schema's existing precedent for invariants like this.
...
- **business_settings** — key/value store (business name, address, GSTIN, invoice prefix, `logo_url`/`logo_public_id` for the business logo, etc.) — replaces the old base64-logo-in-settings approach.
